...
float jaccard_distance_ref_ppc (const float* x, const float* y, size_t d)
{
    //    printf("Optimized jaccard_distance_ref_ppc\n");
    ...
    for (size_t i = 0; i < base; i = i + FLOAT_VEC_SIZE) 
        {
            vx = (vector float *)(&x[i]);
            vy = (vector float *)(&y[i]);
            vaccu_num = vec_add(vaccu_num, vec_min(vx[0], vy[0]));
            vaccu_den = vec_add(vaccu_den, vec_max(vx[0], vy[0]));
        }            
